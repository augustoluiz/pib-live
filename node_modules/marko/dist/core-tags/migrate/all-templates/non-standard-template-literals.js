    ...
    enter(node) {
      if (node.type === "TemplateLiteral" && node.nonstandard) {
        ...
        if (node.expressions.length === 1 && !node.quasis.join("")) {
          ...
          node.expressions = node.expressions.map(expr => builder.conditionalExpression(builder.binaryExpression(expr, "==", builder.literalNull()), builder.literal(""), expr));
          literals.push(node);
        }
      }
    }
    ...
    context.addMigration({
      ...
      apply(helpers) {
        return helpers.prompt({
          type: "confirm",
          message: "Non standard template literals convert null/undefined/false to empty strings, JavaScript template literals do not. If you are relying on this it may cause issues. Would you like to attempt to migrate?",
          initial: true
        }).then(shouldMigrate => {
          if (shouldMigrate) {
